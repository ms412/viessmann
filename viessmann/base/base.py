...
class ViBase(object):
    ...
    def connect(self, username, password, token_file=None):
    ...
    def getNewToken(self, username, password, token_file=None):
        ...
        try:
            ...
            oauth.fetch_token(token_url, client_secret=client_secret, authorization_response=authorization_url,
                              code=codestring)
        ...
        try:
            ...
            try:
                ...
                if j.status_code == 204:
                    ...
                    return json.loads(
                        "{\"statusCode\":" + j.status_code + ", \"error\": \"Unknown\", \"message\": \"UNKNOWN\"}")
        except TokenExpiredError as e:
    ...
    def getProperty(self, property_name):
        url = apiURLBase + '/operational-data/installations/' + str(self.id) + '/gateways/' + str(
            self.serial) + '/devices/0/features/' + property_name
        j = self.__get(url)
        return j

    def setProperty(self, property_name, action, data):
        url = apiURLBase + '/operational-data/v1/installations/' + str(self.id) + '/gateways/' + str(
            self.serial) + '/devices/0/features/' + property_name + "/" + action
